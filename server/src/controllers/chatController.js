...
export const getStockChatToken = async (req, res) => {
  try {
    const user = req.user;
    ...
    const conversation =
      ...
      (await twilioClient.conversations.v1
        ...
        .conversations.create({
          uniqueName,
          friendlyName: `${symbol} Stock Chat`,
        }));
    ...
    console.error("Stock chat token error:", error.message);
    res.status(500).json({
      message: "Failed to generate stock chat token",
      error: error.message,
    });
  }
};
